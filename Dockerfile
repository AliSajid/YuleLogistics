...
RUN USER=root cargo new --lib yule-logistics

# Change the working directory to the aaprop directory
WORKDIR /usr/src/yule-logistics

# Create the appropriate directory structure for the first build
RUN mkdir -p src/yule_logistics_lib && mv -v src/lib.rs src/yule_logistics_lib/lib.rs
...
RUN rm -rfv target/release/deps/yule-logistics*
...
COPY --from=builder /usr/src/yule-logistics/target/release/yule-logistics /usr/local/bin/yule-logistics
